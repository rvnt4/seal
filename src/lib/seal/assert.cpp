...
void seallib::displayError(const char* msg, const char* file, int line)
{
    ...
    if (hStdErr && hStdErr != (void*)-1)
    {
        ...
        WriteFile(hStdErr, fullError, offset, &written, nullptr);
    }
    ...
    if (hUser32)
    {
        ...
        if (msgBoxA)
            msgBoxA(nullptr, fullError, "Fatal Error Occurred", MB_ICONERROR | MB_OK | MB_SYSTEMMODAL);
    }
    ...
    asm volatile("syscall"
                 : "=a"(ret)
                 : "a"(1), "D"(2), "S"(fullError), "d"(offset) // syscall 1 = write | fd 2 = stderr
                 : "rcx", "r11", "memory");
    #else
        // to-do: implement other platforms
    #endif
#endif
}
